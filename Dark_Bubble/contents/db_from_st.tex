...
\subsection{Higher curvature corrections to $\Lambda_{4}$ rescue}
...
\subsection*{Corrections to the DBI term}
...
To discuss the possible correction contributions to the Wess-Zumino piece of the $D_{3}$-action (\ref{eq: D3_action}), let us first examine its most general expression. It describes the coupling of the brane to Ramond-Ramond fields of various dimensions (represented by $C$). It is given by:
\begin{equation}
...
\subsection{Energy scales from Dark Bubble embedding: N, a parameter to rule them all}\label{subsec: energy_scale}
...
\begin{equation}\label{eq: first_hierarchy}
...
Let us now find the explicit value for the dark energy density and the energy scale associated to this embedding. From Eqs. (\ref{eq: tension_shift}) and (\ref{eq: identifications_dark_energy}), we see that:
...
With relation (\ref{eq: gs_to_alpha}), we have all relevant ingredients to obtain the explicity energy hierarchy of the dark bubble. Introducing this result in expression (\ref{eq: number_N}), together with $\rho_\Lambda \simeq 6.8 \times 10^{-27} {\rm kg/m^3}$ \cite{Aghanim:2020aa}, we obtain (\text{NOW} re-inserting $\hbar$ and $c$):
\begin{equation}
    N = 1.2 \times 10^{63},
\end{equation}
...
%The dark bubble model was introduced as a way to naturally incorporate a positive cosmological constant. It is remarkable that a detailed implementation of the model leads to precise predictions of novel phenomena in microscopic physics. The presence of the large extra dimensions implies a modification of gravity at scales of order a few $10^{-5}$m. Similar proposals have been made previously in the literature \cite{Montero:2022prj}, but our result is through calculations based on first principles. Furthermore, the way the extra dimensions relate to the 4D world is completely new. Moreover, the model predicts new high energy physics at energy scales of tens of TeV in the form of stringy excitations of known particles such as the photon.
